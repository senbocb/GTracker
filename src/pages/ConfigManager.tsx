...
import { FileCode, Plus, Download, Trash2, Search, Gamepad2, FileUp, FileText } from 'lucide-react';
...
const ConfigManager = () => {
  ...
  const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      reader.onload = (event) => {
        ...
        setNewConfig({ ...newConfig, content, fileName: file.name });
        showSuccess(`File "${file.name}" loaded.`);
      };
  ...
  const handleAdd = () => {
    if (!newConfig.gameId || !newConfig.label || !newConfig.content) {
      showError("Game, Label, and Content are required.");
      return;
    }
    const updated = [
      { ...newConfig, id: Date.now().toString(), timestamp: new Date().toISOString() },
      ...configs
    ];
    setConfigs(updated);
    localStorage.setItem('combat_configs', JSON.stringify(updated));
    setNewConfig({ gameId: '', label: '', content: '', fileName: '' });
    showSuccess("Configuration archived.");
  };
  ...
  const downloadConfig = (config: ConfigFile) => {
    ...
    a.href = url;
    a.download = config.fileName || `${config.label}.cfg`;
  ...
  const filtered = configs.filter(c => 
    ...
    c.gameId.toLowerCase().includes(search.toLowerCase())
  );
      ...
      <main className="max-w-5xl mx-auto p-6 md:p-10">
        <div className="mb-10">
          <h1 className="text-4xl font-black tracking-tight text-white mb-2 italic uppercase flex items-center gap-4">
            <FileCode className="text-indigo-500" size={36} />
            Config Archive
          </h1>
          <p className="text-slate-400 font-medium">Centralized repository for game settings, keybinds, and autoexec files.</p>
        </div>
              ...
              <CardContent className="p-6 space-y-6">
                ...
                <div className="space-y-4">
                  ...
                  <div className="space-y-2">
                    ...
                    <Input 
                      placeholder="e.g. Pro Settings v2" 
                  ...
                  <div className="space-y-2">
                    <Label className="text-[10px] font-bold uppercase text-slate-500">Config Content</Label>
                    <div className="space-y-2">
                      <Button 
                        variant="outline" 
                        className="w-full border-dashed border-slate-800 bg-slate-950/50 text-slate-400 hover:text-white"
                        onClick={() => fileInputRef.current?.click()}
                      >
                        <FileUp size={16} className="mr-2" />
                        {newConfig.fileName ? newConfig.fileName : 'Upload .cfg / .ini'}
                      </Button>
                      <input type="file" ref={fileInputRef} className="hidden" onChange={handleFileUpload} />
                      <Textarea 
                        placeholder="Or paste raw config text here..." 
                        value={newConfig.content}
                        onChange={(e) => setNewConfig({...newConfig, content: e.target.value})}
                        className="bg-slate-950 border-slate-800 font-mono text-[10px] h-32"
                      />
                    </div>
                  </div>
                  <Button onClick={handleAdd} className="w-full bg-indigo-600 hover:bg-indigo-500 font-black uppercase py-6">
                    Archive Config
                  </Button>
                </div>
              </CardContent>
              ...
              {filtered.length > 0 ? filtered.map((item) => (
                <div key={item.id} className="p-5 rounded-2xl bg-slate-900/90 border border-slate-800 group hover:border-indigo-500/30 transition-all flex items-center justify-between">
                  <div className="flex items-center gap-4">
                    <div className="w-12 h-12 rounded-xl bg-slate-950 flex items-center justify-center text-indigo-500 border border-slate-800">
                      <FileText size={24} />
                    </div>
                    <div>
                      <div className="flex items-center gap-2">
                        <h3 className="text-base font-black text-white uppercase italic tracking-tight">{item.label}</h3>
                        <span className="px-2 py-0.5 rounded bg-indigo-600/10 border border-indigo-500/20 text-[9px] font-black text-indigo-400 uppercase">
                          {item.gameId}
                        </span>
                      </div>
                      <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest mt-1">
                        {item.fileName || 'Raw Text'} • Archived {new Date(item.timestamp).toLocaleDateString()}
                      </p>
                    </div>
                  </div>
                  <div className="flex items-center gap-2">
                    <Button 
                      ...
                      className="h-10 w-10 text-slate-400 hover:text-white hover:bg-indigo-600/20"
                    >
                      <Download size={18} />
                    </Button>
                    <Button 
                ...
                <div className="p-12 text-center border border-dashed border-slate-800 rounded-3xl bg-slate-900/20">
                  <p className="text-xs font-bold text-slate-600 uppercase tracking-widest">Archive is empty</p>
                </div>
              )}
            </div>
          </div>
