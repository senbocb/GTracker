...
import { Target, Plus, Copy, Trash2, Search, Gamepad2, Tag } from 'lucide-react';
...
const CrosshairManager = () => {
  ...
  const handleAdd = () => {
    ...
    const updated = [
      { ...newCrosshair, id: Date.now().toString(), timestamp: new Date().toISOString() },
      ...crosshairs
    ];
    setCrosshairs(updated);
    localStorage.setItem('combat_crosshairs', JSON.stringify(updated));
    setNewCrosshair({ gameId: '', label: '', code: '' });
    showSuccess("Crosshair archived in vault.");
  };

  const removeCrosshair = (id: string) => {
    const updated = crosshairs.filter(c => c.id !== id);
    setCrosshairs(updated);
    localStorage.setItem('combat_crosshairs', JSON.stringify(updated));
    showSuccess("Crosshair removed from vault.");
  };
    ...
    <AppLayout>
      <main className="max-w-5xl mx-auto p-6 md:p-10">
        <div className="mb-10">
          ...
          <p className="text-slate-400 font-medium">Secure storage for tactical reticle configurations across all operations.</p>
        </div>
              ...
              <CardContent className="p-6 space-y-6">
                ...
                <div className="space-y-4">
                  ...
                  <div className="space-y-2">
                    ...
                    <Input 
                      placeholder="e.g. Tournament Dot" 
                      value={newCrosshair.label}
                      onChange={(e) => setNewCrosshair({...newCrosshair, label: e.target.value})}
                      className="bg-slate-950 border-slate-800"
                    />
                  </div>
                  <div className="space-y-2">
                    <Label className="text-[10px] font-bold uppercase text-slate-500">Crosshair Code</Label>
                    <Input 
                      placeholder="Paste code here..." 
                      value={newCrosshair.code}
                      onChange={(e) => setNewCrosshair({...newCrosshair, code: e.target.value})}
            ...
            <div className="grid grid-cols-1 gap-4">
              {filtered.length > 0 ? filtered.map((item) => (
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <div className="flex items-center gap-2">
                        ...
                        <span className="px-1.5 py-0.5 rounded bg-indigo-600/10 border border-indigo-500/20 text-[8px] font-black text-indigo-400 uppercase">
                          {item.gameId}
                        </span>
                      </div>
                      <p className="text-[10px] font-mono text-slate-500 mt-1 truncate max-w-[200px] md:max-w-xs">{item.code}</p>
                    </div>
                  </div>
                  <div className="flex items-center gap-2">
                    <Button 
                    ...
                    <Button 
                      variant="ghost" 
                      size="icon" 
                      onClick={() => removeCrosshair(item.id)}
                ...
                <div className="p-12 text-center border border-dashed border-slate-800 rounded-3xl bg-slate-900/20">
                  <p className="text-xs font-bold text-slate-600 uppercase tracking-widest">Vault is empty</p>
                </div>
              )}
            </div>
          </div>
