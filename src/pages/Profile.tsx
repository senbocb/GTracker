...
import { 
  User, Edit3, Save, Camera, Loader2, ImageIcon, Shield, ChevronRight, Globe, Zap, Target, Activity, Download
} from 'lucide-react';
...
const Profile = () => {
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      link.download = `${editedProfile.username}_combat_profile.png`;
      link.href = canvas.toDataURL('image/png');
      link.click();
      showSuccess("Profile card exported.");
    } catch (err) {
  ...
  return (
    <AppLayout>
      <main className="max-w-7xl mx-auto p-4 md:p-10 space-y-10">
        <div ref={exportRef} className="space-y-10 p-4 rounded-[3rem]">
          {/* Banner Section */}
